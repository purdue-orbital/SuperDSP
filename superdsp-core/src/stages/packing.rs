...
/// Packs a vector of bytes into a new vector using a specified number of bits per element.
///
/// # Arguments
///
/// * `bytes` - A mutable reference to a vector of bytes that will be packed.
/// * `N` - The number of bits to use for each element during packing.
pub fn pack<const N: u8>(mut bytes: ResMut<Vec<u8>>) {
    *bytes = __pack__(bytes.as_slice(), N);
}

/// Unpacks a vector of bytes where each byte contains packed elements.
/// Each element is unpacked into `N` bits, and the result is stored in a new vector.
///
/// # Arguments
///
/// * `bytes` - A mutable reference to a vector of bytes (`ResMut<Vec<u8>>`) that will be modified to contain the unpacked elements.
