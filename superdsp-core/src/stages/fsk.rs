...
pub fn intermediate_sequence_f32(
    x: &[Complex<f32>],
    n: isize,
    radians_a_sample: f32,
...
impl FSKSettings {
    pub fn create_packet_detection_settings(
        &self,
        preamble: &[u8],
        sps: usize,
    ) -> PacketDetectionSettingsF32 {
        let mut temp = Vec::with_capacity(preamble.len() * 8);
        let mut modded = vec![Complex::zero(); 1];

        let mut matrix: DMatrix<Complex<f32>> = DMatrix::zeros(sps, 1);
        ...
        PacketDetectionSettingsF32 {
            ...
            buffer: DMatrix::zeros(0, 0),
        }
    }
}

/// Does 1 bit at a time
pub fn fsk_demod_f32(
    fsk_settings: Res<FSKSettings>,
    input: Res<Vec<Complex<f32>>>,
    mut output: ResMut<u8>,
) {
    let c0 = goertzel_algorithm_f32(
        input.as_slice(),
        fsk_settings.channel_0,
        fsk_settings.radians_a_sample_c0,
    );
    let c1 = goertzel_algorithm_f32(
        input.as_slice(),
        fsk_settings.channel_1,
        fsk_settings.radians_a_sample_c1,
    );
...
fn __fsk_mod__(fsk_settings: &mut FSKSettings, input: u8, output: &mut [Complex<f32>]) {
    if input == 1 {
        for x in output.iter_mut() {
            *x = Complex::new(
                cosf(fsk_settings.c_radian_c1),
                sinf(fsk_settings.c_radian_c1),
            );
        ...
        for x in output.iter_mut() {
            *x = Complex::new(
                cosf(fsk_settings.c_radian_c0),
                sinf(fsk_settings.c_radian_c0),
            );
