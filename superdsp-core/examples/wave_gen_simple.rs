...
use superdsp_core::stages::wave_gen::{wave_gen_f32, WaveGenInformation};

fn configure(mut wave_gen_information: ResMut<WaveGenInformation>, mut dsp_information: ResMut<DspInformation>, mut arr: ResMut<Vec<f32>>) {
...
fn print_wav_gen(arr: Res<Vec<f32>>){
    for x in arr.iter(){
        print!("{:.2} ", x);
    }
}
...
fn main() {
    ...
    s.add_resource(vec![1.0 as f32]);
    
    s.add_stage(Startup, configure);
    
    s.add_stage(Update, wave_gen_f32);
