...
use superdsp_core::stages::fsk::{FSKSettings, fsk_mod_f32};
use superdsp_core::stages::packet_detection::{PacketDetectionSettingsF32, cross_correlation_f32};
...
pub fn setup(
    ...
    mut fsksettings: ResMut<FSKSettings>,
    mut pds: ResMut<PacketDetectionSettingsF32>,
) {
...
pub fn signal_print(signal: Res<Complex<f32>>) {
...
fn main() {
    ...
    s.add_stage(Update, reset);
    s.add_stage(Update, unpack::<1>);
    s.add_stage(Update, fsk_mod_f32);
    s.add_stage(Update, cross_correlation_f32);
